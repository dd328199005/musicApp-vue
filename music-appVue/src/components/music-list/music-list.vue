...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted() {
      ...
      this.$refs.list.$el.style.top = `${this.maxHeight}px`// $el是针对组件的根元素
    },
    created() {
      this.probeType = 3
    ...
    watch: {
      scrollY(newVal) {
        let height = Math.min(newVal, this.maxHeight - STATE_HEIGHT)
        this.$refs.layer.style[transform] = `translate3d(0, -${height}px, 0)`
        let zIndex = 0
        let scale = 1
        let percent = Math.abs(newVal / this.maxHeight)
        let blur = 0
        ...
        if (newVal > (this.maxHeight - STATE_HEIGHT)) { // 滚动到顶部
          this.$refs.bgImage.style.paddingTop = 0
          this.$refs.bgImage.style.height = `${STATE_HEIGHT}px`
          zIndex = 10
